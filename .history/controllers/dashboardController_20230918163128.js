    ...
    try {
      ...
      for (let i = 0; i < req.query.categories.length; i++) {
        const newUser = await prisma.user.create({
          data: {
            // Other user properties you want to set
            // ...
      
            // Connect each user to the specific role by specifying its ID in the roles relation
            roles: {
              connect: { id: roleId },
            },
          },
          include: {
            roles: true, // Include the connected role in the result if needed
          },
        });
      return res.status(201).json(categories);
    } catch (err) {
